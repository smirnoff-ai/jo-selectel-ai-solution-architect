...
Если есть агент — второй sequence: lazy create чата (POST только при первой отправке, если так решили в ux-logic); ветка stream (подписка, события, end); ветка invoke.

Если есть RAG — на диаграмме только узлы Source → Index → Tool; детали в [rag.md](rag.md).
