...
## 5a. Агент и LLM (если есть)

Детали — в agent-harness.md, generation.md, agent-security.md.

| Вопрос | Короткий ответ | Документ |
|--------|----------------|----------|
| Есть агент | да / нет | harness |
| Runtime: invoke / stream / cancel | … | generation |
| Путь к LLM (SDK, gateway, где модели) | одна строка | harness §6 |
| Три контура (UI / checkpoint / observability) | да, разведены | harness §10, data-model |
| Профиль | id / — | шапка harness |
...
Каталог моделей и путь к LLM не дублировать — ссылка на `agent-harness.md`.
