...
| Вывод модели | да | исход на карточке пишет модель через `update_card`; код его не переписывает |
| Аргументы tool | да | Pydantic, `extra=forbid` |
...
| # | Угроза | Реакция | Как |
|---|--------|---------|-----|
| T1 | Jailbreak в письме | mitigate | промпт; исход только из `card.decision` |
...
| T4 | Заявка на площадку без актива | by design | `not_found` + исход clarify, S4 |
