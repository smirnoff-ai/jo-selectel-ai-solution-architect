"""Аргументы тула update_card. Канон схемы.
...
class RecordRef(BaseModel):
    ...
    system: SystemName = Field(description="Справочник, из которого только что пришёл идентификатор")
...
class EvidencePatch(BaseModel):
    ...
    confidence: Confidence = Field(description="high — явно в тексте; medium или low — домысливание")
...
class BindingPatch(BaseModel):
    ...
    id: str | None = Field(default=None, description="Идентификатор при resolved, иначе не передавай")
...
class ContractPatch(BaseModel):
    ...
    site_id: str | None = Field(default=None, description="Площадка, для которой спрашивали договор")
...
class DecisionPatch(BaseModel):
    ...
    reply_draft: str | None = Field(default=None, description="Черновик ответа клиенту, не отправлять")
