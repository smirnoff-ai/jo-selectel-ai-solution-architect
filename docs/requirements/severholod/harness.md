...
> Черновик пакета заказчика. Актуал реализации — [concept/agent-harness.md](../../concept/agent-harness.md).
...
| Этот harness | system prompt, описания тулов, расчёт, сборка draft по `card.decision` |
...
| `stream` | да | Основной UI: мысль, вызов тула, ответ тула, патч карточки, markdown модели, конец прогона |
...
| Structured output | Нет. Исход модель пишет в `card.decision` через `update_card`; в чат — свой markdown |
| Эмбеддинги | Нет |
...
Шесть тулов. В брифе были `find_customer_or_site`, `find_assets`, `get_contract`, `find_open_tickets`, `create_or_update_ticket`. Создание заявки модель не вызывает: примерку делает код после прогона, если на карточке create/update и опора ясна.
...
| `search_tickets` | агент | как `GET /itsm/v1/tickets`, любой один из `customer_id` / `site_id` / `asset_id` / `contract_id` | Найти открытые заявки, только чтение | Вызов без устойчивого фильтра |
...
### 9.4. Стоп прогона, без двери

Когда модель больше не зовёт тулы, она пишет решение в карточку (`update_card` → `decision`) и отдаёт markdown диспетчеру. Код исход не переписывает и отчёт не собирает.

После прогона смотрим `card.decision`.

- На карточке `create` или `update`, клиент и объект `resolved`, для update есть открытый тикет, для create его нет, актив не висит в `ambiguous` / `not_found` — собираем `ticket_draft` и делаем примерку ITSM. `auto_in_prod` true, если примерка принята.
- На карточке create/update, а опора не такая — заявку не трогаем, исход на карточке остаётся как написала модель, `auto_in_prod` false.
- Любой другой исход или пустой `outcome` — без примерки, `auto_in_prod` false.
...
Чат на карточке по-прежнему показывает ход диспетчеру. Langfuse — для нас: тулы, токены, какой исход модель положила на карточку. Датасеты — когда будет [scenarios.md](scenarios.md).
...
Промпт плюс конструкция. Id на слотах пишет агент через `update_card`, и только если этот id был в ответе поиска прогона. Payload заявки собираем мы, не модель. Примерка ITSM — только после прогона и только если на карточке create/update и опора ясна. Исходящих в канал нет.
