...
```json
{
  ...
  "grounds": ["Два актива с одним кодом на разных площадках, выбрать нельзя"],
  "ticket_draft": null,
  "itsm_dry_run": null,
  "auto_in_prod": false,
  "questions": [
...
`grounds` — короткие фразы модели, на каком основании исход, не пути слотов.

---

## 8. Целиком: шаблон `card`

Поля, которые код создаёт в момент приёма. Агент пишет факты, привязки, договор, расчёт и решение через `update_card`. Код после прогона исход не переписывает; при create/update собирает примерку заявки. `schema_version` нужен, чтобы не гадать над старыми снимками.
