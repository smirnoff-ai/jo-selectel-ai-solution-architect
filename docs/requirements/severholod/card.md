...
Поля при `resolved`: `id`, `site_id`, `plan`, `response_sla`, `service_window`, `coverage`. Пишет код после тула договора, не модель.
...
Пишет правило, не модель. Модель может положить желаемый срок в факты; числа приоритета, код SLA и дедлайн она не назначает.
...
Поля, которые код создаёт в момент приёма. Агент потом патчит факты; код дописывает привязки, договор, расчёт и принимает решение. `schema_version` нужен, чтобы не гадать над старыми снимками.
